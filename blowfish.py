...
Modified by Stephen Haywood <averagesecurityguy.info> to include test vectors
from http://www.schneier.com/code/vectors.txt and to support a 1 byte key,
which is expected based on the test vectors.
"""
...
class Blowfish:
	...
	def __init__ (self, key):
		...
		if not key or len (key) < 1 or len (key) > 56:
...
if __name__ == '__main__':
	...
	for test in tests:
		...
		teststr = "Testing KEY: " + k.encode("hex")
		teststr += " with PLAIN: " + p.encode("hex")
		if e == c:
			teststr += " - Results: Pass"
		else:
			teststr += " - Results: Fail"
